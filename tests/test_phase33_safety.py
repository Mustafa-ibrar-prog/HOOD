"""Phase 33 Final Safety Check (Part M/24): corrected multiple-testing
accounting and P22-OPT-013 coarse-grained replication ONLY -- no live
order, no paper order, no autonomous live trading activation, no new
trading STRATEGY object, no paid data purchase, no ORATS activation, no
imputation/interpolation of missing observations, no execution-gateway
modification.

Same AST-based string/comment-blanking technique established in Phase
28-32 -- forbidden-pattern checks scan real code structure only, never
...
PHASE33_SRC_MODULES = [
    "src/options/phase33_test_registry.py",
    "src/options/phase33_corrected_campaign.py",
    "src/options/phase33_range_expansion_feature.py",
    "src/options/phase33_group_balanced_evidence.py",
    "src/options/phase33_replication_robustness.py",
    "src/options/phase33_replication_hypotheses.py",
    "src/options/phase33_replication_campaign.py",
]
...
def _all_phase33_files():
    return [REPO_ROOT / rel for rel in PHASE33_SRC_MODULES]
...
def test_phase33_files_exist():
    for rel in PHASE33_SRC_MODULES:
        assert (REPO_ROOT / rel).is_file(), f"missing {rel}"


def test_no_phase33_file_imports_the_live_order_placement_path():
    for path in _all_phase33_files():
...
def test_no_phase33_file_calls_a_live_paper_or_simulated_order_function():
    for path in _all_phase33_files():
...
def test_no_phase33_file_creates_a_strategy_object():
    forbidden_patterns = (
...
def test_no_phase33_file_declares_a_hypothesis_validated():
...
def test_no_phase33_file_records_a_human_authorized_system_state_transition():
    for path in _all_phase33_files():
        source = _code_with_string_literals_and_comments_blanked(path)
        assert "record_human_authorized_transition(" not in source, f"{path} calls record_human_authorized_transition"


def test_no_phase33_file_purchases_or_activates_orats():
...
def test_no_phase33_file_imputes_or_interpolates_missing_data():
    """No manufactured/interpolated observation ever enters this phase's
    range-expansion feature or replication targets -- every aggregate is
    None (DATA_LIMITED) when real data is insufficient, never a
    forward-filled or interpolated substitute."""
...
def test_five_replication_hypotheses_registered_this_phase():
    from src.options.phase33_replication_hypotheses import build_hypotheses
    assert len(build_hypotheses()) == 5


def test_discovery_classification_reused_unchanged():
    """Part K: 'Use ONLY existing classifications... No weaker custom
    category' -- confirmed by reusing the SAME 7-value enum Phase 31/32
    already established, not a redefined one."""
...
def test_correction_families_kept_structurally_separate():
    """Part A/I's core fix: diagnostic and placebo records must never be
    counted inside the primary correction family."""
    from src.options.phase33_test_registry import DIAGNOSTIC_FAMILY, PLACEBO_FAMILY, PRIMARY_FAMILY
    assert len({PRIMARY_FAMILY, DIAGNOSTIC_FAMILY, PLACEBO_FAMILY}) == 3


def test_every_inferential_test_registered_before_correction():
    """Part A's explicit requirement, verified structurally: apply_correction
    only ever reads FROM the registry -- it cannot correct a test that was
    never registered, and `TestRegistry.register` raises on an accidental
    duplicate rather than silently overwriting."""
    from src.options.phase33_test_registry import InferentialTestRecord, PRIMARY_FAMILY, TestRegistry
    reg = TestRegistry()
    reg.register(InferentialTestRecord(
        hypothesis_id="X", feature_family="A", feature="f", target="t", horizon=5, aggregation_method="cross_sectional",
        bucket_definition="fine", underlying="ALL", test_type="ic_p", sample_size=10, p_value=0.01, effect_size=0.1,
    ...
    with pytest.raises(ValueError):
        reg.register(InferentialTestRecord(
            hypothesis_id="X", feature_family="A", feature="f", target="t", horizon=5, aggregation_method="cross_sectional",
            bucket_definition="fine", underlying="ALL", test_type="ic_p", sample_size=10, p_value=0.02, effect_size=0.1,
            correction_family=PRIMARY_FAMILY,
        ))

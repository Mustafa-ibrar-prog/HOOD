...
ORATS_SCORECARD = ProviderScorecard(
    ...
    scores=(
        ...
        _s(D.QUOTE_SIZES, 3, "Strike.call_bid_size/call_ask_size/put_bid_size/put_ask_size confirmed real fields (Phase 29 self-correction: Phase 28's original 1/5 score for this dimension was a real recall error -- these fields ARE present in the verbatim schema this project fetched in Phase 25; see docs/phase29_orats_activation_and_certification.md's correction note)"),
        ...
        _s(D.EXECUTION_REALISM, 3, "real bid/ask price AND size fields confirmed (corrected, see QUOTE_SIZES); no explicit last-trade price/size (tick) field confirmed in the schema -- Strike.call_value/put_value appear to be model/theoretical values, not trade prints, so this stays below ThetaData's tick-trade-confirmed evidence"),
...
THETADATA_SCORECARD = ProviderScorecard(
    ...
    scores=(
        ...
        _s(D.QUOTE_SIZES, 3, "Quote.bid_size/ask_size confirmed real fields (ORATS also has confirmed size fields -- see its corrected score; this is no longer a unique ThetaData advantage)"),
